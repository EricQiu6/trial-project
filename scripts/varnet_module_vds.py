...
class LatentVarNet(nn.Module):
    ...
    def forward(
        ...
        mask: torch.Tensor,
        num_low_frequencies: Optional[int] = None,
        latent_vector: Optional[torch.Tensor] = None,  # New: latent vector input
        sens_maps: Optional[torch.Tensor] = None,  # Accept precomputed maps
    ) -> torch.Tensor:
        ...
        for cascade in self.cascades:
            kspace_pred = cascade(
                ...
                mask, 
                sens_maps,
                latent_vector  # Pass latent_vector to each block
            )
...
class VarNetBlock(nn.Module):
    ...
    def forward(
        ...
        latent_vector: torch.Tensor,  # New: latent vector input
    ) -> torch.Tensor:
        zero = torch.zeros(1, 1, 1, 1, 1).to(current_kspace)
        soft_dc = torch.where(mask, current_kspace - ref_kspace, zero) * self.dc_weight
        model_term = self.sens_expand(
            self.model(self.sens_reduce(current_kspace, sens_maps), latent_vector),  # Pass latent_vector
            sens_maps
        )
